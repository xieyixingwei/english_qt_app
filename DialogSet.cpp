...
DialogSet::DialogSet()
{
    ...
    m_ui->lwdg_searchfiles->addItems(Settings::ToStringList(SETS.GetGroup(GROUP_SEARCH_FILES)));
...
void DialogSet::Ok_Btn_Slot()
{
    ...
    SETS.RemoveGroup(GROUP_SEARCH_FILES);
    for(int i = 0; i < m_ui->lwdg_searchfiles->count(); i++)
    {
        QString pathfile = m_ui->lwdg_searchfiles->item(i)->text().trimmed();
        if(!pathfile.isEmpty())
        {
            SETS.SetValue(QString(GROUP_SEARCH_FILES) + "/" + QString::number(i), pathfile);
        }
    }
